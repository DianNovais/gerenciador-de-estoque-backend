...
export class sellController{
    public static async sellCreate(req: Request, res: Response){
        ...
        if(!cartExist || cartExist.products.length < 1){
            return res.status(404).json({"message": "adicione produtos ao carrinho!"});
        }
        ...
        try{
            ...
            const allProducts = await product.find({_id: { $in: mapCartIds}}).select('value name quantity');
            const mapProducts = new Map(allProducts.map(p => [p._id.toString(), p]));
